...
class KickGroupMemberUseCaseTest {
    ...
    @InjectMocks private lateinit var kickGroupMemberUseCase: KickGroupMemberUseCase

    @Test
    fun `should kick a member successfully`() {
        // Given
        ...
        // When
        kickGroupMemberUseCase.execute(ADMIN_EMAIL, GROUP_ID, MEMBER_USER_ID)
    ...
    fun `should throw NotGroupMemberException when the acting user is not a member`() {
        ...
        assertThrows<NotGroupMemberException> {
            kickGroupMemberUseCase.execute(OUTSIDER_EMAIL, GROUP_ID, MEMBER_USER_ID)
        }
        verify(groupRepository, never()).save(org.mockito.kotlin.any())
    }
    ...
    fun `should throw GroupMemberNotAdminException when the acting user is not an admin`() {
        ...
        assertThrows<GroupMemberNotAdminException> {
            kickGroupMemberUseCase.execute(MEMBER_EMAIL, GROUP_ID, ADMIN_USER_ID)
        }
        verify(groupRepository, never()).save(org.mockito.kotlin.any())
    }
    ...
    fun `should throw NotGroupMemberException when the target user is not a member`() {
        ...
        // When + Then
        assertThrows<NotGroupMemberException> {
            kickGroupMemberUseCase.execute(ADMIN_EMAIL, GROUP_ID, OUTSIDER_USER_ID)
        }
        verify(groupRepository, never()).save(org.mockito.kotlin.any())
    }

    @Test
    fun `should throw LastGroupAdminException when kicking the only admin`() {
        // Given
        val group = createGroupWithAdminAndMember()
        given(groupRepository.findById(GROUP_ID)).willReturn(group)
        given(userFacade.getUserIdByEmail(ADMIN_EMAIL)).willReturn(ADMIN_USER_ID)
        ...
        assertThrows<LastGroupAdminException> {
            kickGroupMemberUseCase.execute(ADMIN_EMAIL, GROUP_ID, ADMIN_USER_ID)
        }
        verify(groupRepository, never()).save(org.mockito.kotlin.any())
    }

    @Test
    fun `should kick an admin successfully when another admin exists`() {
        // Given
        ...
        // When
        kickGroupMemberUseCase.execute(ADMIN_EMAIL, GROUP_ID, OTHER_ADMIN_USER_ID)

        // Then
        val captor = argumentCaptor<Group>()
        verify(groupRepository).save(captor.capture())
        assertFalse(captor.firstValue.isMember(OTHER_ADMIN_USER_ID))
    }
