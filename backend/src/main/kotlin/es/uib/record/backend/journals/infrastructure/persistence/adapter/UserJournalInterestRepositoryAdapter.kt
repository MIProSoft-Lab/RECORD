...
class UserJournalInterestRepositoryAdapter(
    ...
    override fun add(userId: UUID, journalId: UUID) {
        if (!this.springDataJpaUserJournalInterestRepository.existsByUserIdAndJournalId(
                userId,
                journalId,
            )
        ) {
    ...
    override fun remove(userId: UUID, journalId: UUID) {
        this.springDataJpaUserJournalInterestRepository.deleteByUserIdAndJournalId(userId, journalId)
    }
