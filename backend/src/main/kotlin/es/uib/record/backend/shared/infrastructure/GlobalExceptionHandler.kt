...
class GlobalExceptionHandler {
    ...
    fun handleDomainException(e: DomainException): ResponseEntity<ErrorResponse> {
        ...
        val response =
            ErrorResponse(message = e.message, code = e.code, params = e.params.ifEmpty { null })
    ...
    fun handleValidationException(
        ...
        val response =
            ErrorResponse(
                message =
                    "Validation Failed: ${e.bindingResult.fieldErrors.joinToString { "${it.field}: ${it.defaultMessage}" }}",
                code = "",
            )
    ...
    fun handleExpiredJwtException(): ResponseEntity<ErrorResponse> {
        val response = ErrorResponse(message = "JWT token has expired", code = "JWT_EXPIRED")
